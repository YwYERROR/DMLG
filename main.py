...
class Processor():
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            best_test = {"wer": 200.0, "del": 100.0, "ins": 100.0}
            best_epoch = 0
            total_time = 0
            epoch_time = 0
            if is_main_process():
                ...
                if is_main_process():
                    if eval_model:
                        ...
                        self.recoder.print_log(
                            "Dev WER: {:05.2f}% DEV del: {:05.2f}% DEV ins: {:05.2f}%".format(dev_wer['wer'],
                                                                                              dev_wer['del'],
                                                                                              dev_wer['ins']))
                        self.recoder.print_log(
                            "Test WER: {:05.2f}% Test del: {:05.2f}% Test ins: {:05.2f}%".format(test_wer['wer'],
                                                                                                 test_wer['del'],
                                                                                                 test_wer['ins']))
                        dev_wer_formatted = {k: round(v, 2) for k, v in dev_wer.items()}
                        test_wer_formatted = {k: round(v, 2) for k, v in test_wer.items()}
                    ...
                    self.recoder.print_log(
                        'Best_dev: {:05.2f}%, {:05.2f}%, {:05.2f}%, Best_test: {:05.2f}%, {:05.2f}%, {:05.2f}%, Epoch : {}'.format(
                            best_dev["wer"], best_dev["del"], best_dev["ins"], best_test["wer"], best_test["del"],
                            best_test["ins"], best_epoch))
    ...
    def save_model(self, epoch, save_path):
        model = self.model.module if len(self.device.gpu_list) > 1 else self.model
        torch.save({
            'epoch': epoch,
